...
bool Demux::init(const std::string URL, bool isMainDemux) {
    ...
    if (m_lowLatencyMode) {
        // 设置低延迟选项
        QString fflags = "";
        if (m_noBuffer) fflags += "nobuffer";
        if (m_discardCorrupt) {
            if (!fflags.isEmpty()) fflags += "+";
            fflags += "discardcorrupt";
        }
        if (!fflags.isEmpty()) {
            av_dict_set(&opts, "fflags", fflags.toUtf8().constData(), 0);
        }
    ...
    // 设置格式标志位（低延迟配置）
    if (m_lowLatencyMode && m_formatCtx) {
        if (m_noBuffer) {
            m_formatCtx->flags |= AVFMT_FLAG_NOBUFFER;
        }
#ifdef AVFMT_FLAG_LOW_DELAY
        if (m_lowDelayFlag) {
            m_formatCtx->flags |= AVFMT_FLAG_LOW_DELAY;
        }
#endif
#ifdef AVFMT_FLAG_DISCARD_CORRUPT
        if (m_discardCorrupt) {
            m_formatCtx->flags |= AVFMT_FLAG_DISCARD_CORRUPT;
        }
#endif
        if (m_noParse) {
            m_formatCtx->flags |= AVFMT_FLAG_NOPARSE;
        }
    }
...
bool Demux::switchStream(MediaType type, int streamIdx, weakPktQueue wpq, weakFrmQueue wfq) {
    ...
    case MediaType::Video:
        ...
        // 更新视频编码格式和封装格式
        PlaybackStats::instance().videoCodecName = avcodec_get_name(videoStream->codecpar->codec_id);
        if (m_formatCtx->iformat && m_formatCtx->iformat->name) {
            PlaybackStats::instance().containerFormat = m_formatCtx->iformat->name;
        }
        break;
    case MediaType::Audio:
        idxVec = &m_audioIdx;
