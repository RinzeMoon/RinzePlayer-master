...
void VideoPlayer::nextFrame() {
    ...
    // 触发单帧播放
    m_stepFrame.store(true, std::memory_order_relaxed);
}

void VideoPlayer::prevFrame() {
    // 确保是暂停状态
    m_paused.store(true, std::memory_order_relaxed);
    // 上一帧实现比较复杂，需要seek，这里先留空
    // TODO: 实现真正的上一帧功能
    qDebug() << "上一帧功能待实现";
}
...
void VideoPlayer::playerLoop() {
    ...
    while (!m_stop.load(std::memory_order_relaxed)) {
        ...
        if (!m_forceRefresh && m_paused.load(std::memory_order_relaxed)) {
            // 检查是否是单帧模式
            if (m_stepFrame.load(std::memory_order_relaxed)) {
                m_stepFrame.store(false, std::memory_order_relaxed);
                // 继续执行到下一帧
            } else {
                std::this_thread::sleep_for(std::chrono::milliseconds(5));
                continue;
...
bool VideoPlayer::write(AVFrmItem &videoFrmitem) {
    ...
    double speed = GlobalClock::instance().speed();
    // 实际sleep时间 = dt / speed（比如speed 2.0，只需要sleep一半的时间）
    if (dt > 0 && speed > 0.001) {
        double sleepTime = dt / speed;
        if (sleepTime > 0.1) {
            sleepTime = 0.1;
            m_renderTime = nowTime + dt; // 保持renderTime不变
        }
