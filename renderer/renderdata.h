...
// 运动向量数据结构
struct MotionVector {
    int16_t dst_x;     // 目标块中心 X 坐标
    int16_t dst_y;     // 目标块中心 Y 坐标
    int16_t motion_x;  // X 方向运动向量
    int16_t motion_y;  // Y 方向运动向量
    uint8_t w;         // 块宽度
    uint8_t h;         // 块高度
    uint8_t source;    // 参考帧方向 (-1=前向, 1=后向)
    uint8_t flags;     // 其他标志
};
