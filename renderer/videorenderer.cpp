...
VideoRenderer::VideoRenderer() {
    ...
    // --- 初始化运动向量着色器 ---
    const QString mvVertPath = ":/shaderSource/motion_vector.vert";
    const QString mvGeomPath = ":/shaderSource/motion_vector.geom";
    const QString mvFragPath = ":/shaderSource/motion_vector.frag";
    if (!m_mvProgram.addCacheableShaderFromSourceFile(QOpenGLShader::Vertex, mvVertPath) ||
        !m_mvProgram.addCacheableShaderFromSourceFile(QOpenGLShader::Geometry, mvGeomPath) ||
        !m_mvProgram.addCacheableShaderFromSourceFile(QOpenGLShader::Fragment, mvFragPath) ||
        !m_mvProgram.link()) {
        qDebug() << "运动向量着色器程序编译失败";
    } else {
        qDebug() << "运动向量着色器程序编译成功";
    }
    float vertices[] = {
        // 位置      // 纹理
        -1.f, 1.f, 0.f, 1.f,  // 左上
    ...
    // --- 初始化运动向量 VAO/VBO ---
    glGenVertexArrays(1, &m_mvVao);
    glBindVertexArray(m_mvVao);
    
    glGenBuffers(1, &m_mvVbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_mvVbo);
    // 顶点结构: pos.x, pos.y, mv.x, mv.y, source
    // location 0: pos (2 floats)
    // location 1: mv (2 floats)
    // location 2: source (1 float)
...
VideoRenderer::~VideoRenderer() {
    ...
    // --- 清理运动向量资源 ---
    if (m_mvVao != 0) {
        glDeleteVertexArrays(1, &m_mvVao);
    }
    if (m_mvVbo != 0) {
        glDeleteBuffers(1, &m_mvVbo);
    }
}
...
void VideoRenderer::render() {
    ...
    int currentColorSpace = 1; // 默认BT.709
    bool currentIsFullRange = false;
    ...
    m_vidData->read([&](VideoRenderData &renData, int) -> bool {
        ...
        if (frm->color_space == AVCOL_SPC_BT709) {
            currentColorSpace = 1;
        } else if (frm->color_space == AVCOL_SPC_BT2020_NCL || frm->color_space == AVCOL_SPC_BT2020_CL) {
            currentColorSpace = 2;
        } else if (frm->color_space == AVCOL_SPC_RGB) {
            currentColorSpace = 0; // 当作RGB处理
        } else {
            currentColorSpace = 0; // 默认BT.601
        }
        
        currentIsFullRange = (frm->color_range == AVCOL_RANGE_JPEG);
    ...
    // 设置颜色空间信息
    m_program.setUniformValue(m_program.uniformLocation("colorSpace"), currentColorSpace);
    m_program.setUniformValue(m_program.uniformLocation("fullRange"), currentIsFullRange);
...
void VideoRenderer::synchronize(QQuickFramebufferObject *item) {
    ...
    // --- 运动向量参数 ---
    m_showMotionVectors = videoWindow->m_showMotionVectors;
    m_motionVectorScale = videoWindow->m_motionVectorScale;
...
// --- 运动向量渲染函数 ---
void VideoRenderer::updateMotionVectorData(VideoRenderData* renderData) {
    if (!renderData || !renderData->hasMotionVectors) {
        m_mvVertexData.clear();
        return;
    }
    
    m_mvVertexData.clear();
    m_mvVertexData.reserve(renderData->motionVectors.size() * 5);
    
    float fw = static_cast<float>(renderData->frmItem.frm->width);
    float fh = static_cast<float>(renderData->frmItem.frm->height);
    
    for (const auto& mv : renderData->motionVectors) {
        // 将像素坐标转换为标准化设备坐标 (-1..1)
        float nx = (2.0f * mv.dst_x / fw) - 1.0f;
        float ny = 1.0f - (2.0f * mv.dst_y / fh);
        ...
        // source
        float src = static_cast<float>(mv.source);
        
        // 添加顶点
        m_mvVertexData.push_back(nx);
        m_mvVertexData.push_back(ny);
        m_mvVertexData.push_back(mvx);
        m_mvVertexData.push_back(mvy);
        m_mvVertexData.push_back(src);
    }
}

void VideoRenderer::renderMotionVectors() {
    if (m_mvVertexData.empty() || !m_showMotionVectors) {
        return;
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, 
                 m_mvVertexData.size() * sizeof(float), 
                 m_mvVertexData.data(), 
                 GL_DYNAMIC_DRAW);
    
    // 设置 uniform
    int loc = m_mvProgram.uniformLocation("uScale");
    m_mvProgram.setUniformValue(loc, m_motionVectorScale);
    
    loc = m_mvProgram.uniformLocation("uResolution");
    m_mvProgram.setUniformValue(loc, QVector2D(static_cast<float>(m_frameSize.width()), 
                                               static_cast<float>(m_frameSize.height())));
    
    // 绘制点（几何着色器会生成线段和箭头）
    glDrawArrays(GL_POINTS, 0, static_cast<int>(m_mvVertexData.size() / 5));
