...
private:
    ...
    // --- 运动向量渲染器 ---
    QOpenGLShaderProgram m_mvProgram;
    GLuint m_mvVao = 0;
    GLuint m_mvVbo = 0;
    std::vector<float> m_mvVertexData;  // 运动向量顶点数据
    
    // 从 VideoRenderData 更新运动向量顶点数据
    ...
    float m_motionVectorScale = 0.1f;
...
public:
    ...
    // --- 运动向量可视化控制 ---
    Q_INVOKABLE void setShowMotionVectors(bool val) { 
        m_showMotionVectors = val; 
        update();
    }
    Q_INVOKABLE void setMotionVectorScale(float scale) { 
        m_motionVectorScale = scale; 
        update();
    }
