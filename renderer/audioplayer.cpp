...
bool AudioPlayer::init(const AVCodecParameters *codecParams, sharedFrmQueue frmBuf) { // TODO ： initial_padding trailing_padding是否要做处理
    ...
    // 初始化混音器
    m_mixer->init(m_swrPar.sampleRate, m_swrPar.ch_layout.nb_channels);
...
void AudioPlayer::writePCM() {
    ...
    if (m_pcmDataSize > 0 && m_pcmDataPtr) {
        int sampleCount = m_pcmDataSize / m_pcmFrameSize;
        int channels = m_swrPar.ch_layout.nb_channels;
        ...
        // 转换为 float 格式用于处理
        m_effectBuffer.resize(totalSamples);
        ...
        for (int i = 0; i < sampleCount; ++i) {
            ...
            for (int ch = 0; ch < channels; ++ch) {
                ...
                if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_FLT) {
                    sample = ((float*)m_pcmDataPtr)[idx];
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_S16) {
                    sample = ((short*)m_pcmDataPtr)[idx] / 32768.0f;
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_S32) {
                    sample = ((int*)m_pcmDataPtr)[idx] / 2147483648.0f;
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_U8) {
                    sample = (((uint8_t*)m_pcmDataPtr)[idx] - 128) / 128.0f;
                }
                
                m_effectBuffer[idx] = sample;
                monoSample += sample;
            }
            
            monoSample /= static_cast<float>(channels);
            monoSample *= 3.0f; // 放大3倍，让频谱更明显！
            audioData.append(monoSample);
        }
        
        emit audioDataReady(audioData, m_swrPar.sampleRate);

        // 应用音频效果器
        m_mixer->process(m_effectBuffer.data(), sampleCount);

        // 转换回原始格式
        for (int i = 0; i < sampleCount; ++i) {
            for (int ch = 0; ch < channels; ++ch) {
                int idx = i * channels + ch;
                float sample = m_effectBuffer[idx];
                
                // 限制在 [-1, 1] 范围内
                sample = qBound(-1.0f, sample, 1.0f);
                
                if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_FLT) {
                    ((float*)m_pcmDataPtr)[idx] = sample;
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_S16) {
                    ((short*)m_pcmDataPtr)[idx] = static_cast<short>(sample * 32767.0f);
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_S32) {
                    ((int*)m_pcmDataPtr)[idx] = static_cast<int>(sample * 2147483647.0f);
                } else if (m_swrPar.sampleFormat == AV_SAMPLE_FMT_U8) {
                    ((uint8_t*)m_pcmDataPtr)[idx] = static_cast<uint8_t>(sample * 127.0f + 128.0f);
                }
            }
        }
    }
